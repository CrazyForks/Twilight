...
| `internal/config` | 读取项目运行目录 `config.toml`、同目录 `config.local.toml` 与 `TWILIGHT_*` 字段级环境变量；运行入口固定使用当前目录 `config.toml`。 |
...
Go 后端继续读取现有 TOML，运行时主配置文件固定为当前工作目录下的 `config.toml`：

1. 未传 `--config` 时读取当前工作目录的 `config.toml`。
2. 显式传 `--config` 时，只允许 `config.toml`、`./config.toml` 或指向同一个当前目录文件的绝对路径。
3. 其它文件名或其它目录的 `config.toml` 会被拒绝，避免 1Panel、systemd 或环境变量把进程带到错误配置。
4. 主配置同目录的 `config.local.toml` 会作为私密覆盖文件加载。
5. 最后由 `TWILIGHT_*` 环境变量覆盖具体字段。

运行入口不会读取 `TWILIGHT_CONFIG_FILE` 指向的其它路径；这样可以保证 1Panel、systemd 和手动启动时都默认使用项目目录下的 `config.toml`。需要临时测试其它配置时，请切换到对应测试目录后再启动进程，不要在生产进程中指向其它路径。
...
| `TWILIGHT_LOG_LEVEL` | 后端日志等级：`debug` / `info` / `warn` / `error`，兼容旧数字 `10/20/30/40`。 |
...
- 生产模板默认推荐 PostgreSQL；Go JSON 状态文件 `db/twilight_go_state.json` 保留为兼容存储。
- 现有 Go JSON 状态文件可通过 `[Database] state_file = "/path/to/twilight_go_state.json"` 或 `TWILIGHT_STATE_FILE` 指定。
- PostgreSQL 后端：配置 `database.driver = "postgres"` 和 DSN，适合用户量、播放记录、调度记录较多的部署。
- SQLite 仅作为旧 Python 数据库迁移来源或手动配置保留，不会在前端可视化配置中作为运行后端选项展示。
...
- 日志等级和保留行数由 `[Global].log_level`、`[Global].runtime_log_limit` 控制，保存配置后会热重载。
- 日志缓冲默认保留最近 5000 条，管理端可继续加载更多历史行，避免长时间运行后撑爆内存或页面。
